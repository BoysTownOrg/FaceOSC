...
static auto putWithTrailingComma(std::ostream &stream, const std::string &s)
    -> std::ostream & {
  return stream << s << ',';
}
...
void ofApp::loadSettings() {
  ...
  const auto videoFilePath = ofSystemLoadDialog("load a video").filePath;
  if (videoFilePath != "") {
    if (!movie.load(videoFilePath)) {
      ofLog(OF_LOG_ERROR, "Could not load movie \"%s\"", videoFilePath.c_str());
    }
  }
  movie.setVolume(1.0);
  movie.setSpeed(1.0);
...
void ofApp::update() {
  ...
  if (videoSource->isFrameNew()) {
    ...
    if (tracker.getFound()) {
      ...
      putWithTrailingComma(outputFile,
                           tracker.getGesture(ofxFaceTracker::MOUTH_WIDTH));
      putWithTrailingComma(outputFile,
                           tracker.getGesture(ofxFaceTracker::MOUTH_HEIGHT));
      putWithTrailingComma(
          outputFile, tracker.getGesture(ofxFaceTracker::LEFT_EYEBROW_HEIGHT));
      putWithTrailingComma(
          outputFile, tracker.getGesture(ofxFaceTracker::RIGHT_EYEBROW_HEIGHT));
      putWithTrailingComma(
          outputFile, tracker.getGesture(ofxFaceTracker::LEFT_EYE_OPENNESS));
      putWithTrailingComma(
          outputFile, tracker.getGesture(ofxFaceTracker::RIGHT_EYE_OPENNESS));
      putWithTrailingComma(outputFile,
...
void ofApp::draw() {
  ...
  if (tracker.getFound()) {
    ofDrawBitmapString("framerate " + ofToString((int)ofGetFrameRate()) +
                           ", sending OSC to " + host + ":" + ofToString(port),
                       10, ofGetHeight() - 4);
  ...
  ofSetColor(255, 0, 0);
  ofDrawBitmapString("speed " + ofToString(movie.getSpeed()),
                     ofGetWidth() - 100, 20);
